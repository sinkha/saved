...
for (Issue issue in issues)
{
	...
	int idx2 = issue.summary.lastIndexOf(" :");
	if (idx != -1) {
		...
		if(idx2!=-1)
			newSummary = issue.summary.substring(idx+3,idx2) + " : " + issue.summary.substring(0, idx) + " : " + issue.summary.substring(idx2+2);
		else {
			newSummary = issue.summary.substring(idx+3) + " : " + issue.summary.substring(0, idx);
		}
		
		newSummary =  newSummary.replaceAll("\\.0\\.", "\\.00\\.");
