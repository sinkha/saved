...
public class XSLTransformerMojo extends AbstractMojo {
   /**
    * Collection of TransformationSets to transform. (TransformationSet contains
    * includes, excludes, includesDirectory, outputDirectory, outputFilename,
    * stylesheet, outputMethod, indentAmount, useSAX, and batchIncludes.)
    * 
    * @parameter
    */
   protected ArrayList<TransformationSet> transformationSets;
   ...
   protected void transform(File file, TransformationSet transformationSet, boolean useTransformationSetFilename) throws Exception {
      ...
      if (useTransformationSetFilename && transformationSet.outputFilename != null)
         outFile = new File(transformationSet.outputDirectory, transformationSet.outputFilename);
      else
         outFile = new File(transformationSet.outputDirectory, file.getName());
   ...
   protected void batchTransform(List<File> files, TransformationSet transformationSet) throws Exception {
      ...
      if (transformationSet.outputFilename != null)
         ...
         outFile = new File(transformationSet.outputDirectory, files.get(0).getName());
      ...
      if (transformationSet.useSAX) {
         ...
         boolean b = true;
         if (b)
            throw new MojoExecutionException("Batching includes using SAX is not currently supported. Try setting TransformationSet.useSAX to false.");
