...
import com.cdmtech.proj.sol.importer.marad.marad_1_0_0.dwg.SOMShipProfileDbDatabaseImporter;
import com.cdmtech.proj.sol.utility.dwg.DWGLoader;
import com.cdmtech.sol.maven.plugin.FileProcessingUtils;

/**
 * Loads a MARAD Ship (profile) DWG CAD file into memory by first converting to
 * XML using an ODA-based (Open Design Alliance) tool that streams the xml to
 * the process' stdout. The streamed XML is then parsed using a StAX parser
 * creating CAD database objects in memory which are in turn processed into
 * SOM objects.
 * 
 * @goal import-marad-ship-profile-dwg
 * @phase generate-resources
 * @author csleight
 */
public class MARADShipProfileDWGToSOMMojo extends AbstractMojo {
   /**
   ...
   public void execute() throws MojoExecutionException {
      try {
         ...
         for (String includedFilePath : includedFilePaths) {
            ...
            SOMShipProfileDbDatabaseImporter importer = new SOMShipProfileDbDatabaseImporter();
